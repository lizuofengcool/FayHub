...
type PluginEngine interface {
	...
	Install(ctx context.Context, tenantID uint, pluginID string, version string, licenseKey string) error

	// Uninstall 卸载插件：关闭 WASM 模块 → 释放 wazero 运行时内存 → 注销路由/API/菜单
	Uninstall(ctx context.Context, tenantID uint, pluginID string) error

	// Enable 启用插件：重新实例化 WASM 模块 → 注册路由/API/菜单
	Enable(ctx context.Context, tenantID uint, pluginID string) error

	// Disable 禁用插件：关闭 WASM 模块实例 → 释放内存 → 注销路由/API/菜单（保留元数据，可再次 Enable）
	Disable(ctx context.Context, tenantID uint, pluginID string) error

	// Upgrade 升级插件：卸载旧版本 → 安装新版本（原子操作，失败则回滚）
	Upgrade(ctx context.Context, tenantID uint, pluginID string, oldVersion string, newVersion string, licenseKey string) error
	...
	Call(ctx context.Context, tenantID uint, pluginID string, functionName string, payload []byte) ([]byte, error)

	// GetStatus 获取插件运行状态（active/disabled/error）
	GetStatus(ctx context.Context, tenantID uint, pluginID string) (*PluginStatus, error)

	// ListPlugins 列出指定租户已安装的所有插件
	ListPlugins(ctx context.Context, tenantID uint) ([]*InstalledPluginInfo, error)

	// HealthCheck 健康检查：验证插件模块是否正常加载、状态是否为 active
	HealthCheck(ctx context.Context, tenantID uint, pluginID string) error
...
func (e *NoopEngine) Install(ctx context.Context, tenantID uint, pluginID string, version string, licenseKey string) error {
	log.Printf("[NoopEngine] 安装插件: tenant=%d, plugin=%s, version=%s", tenantID, pluginID, version)
	return nil
}

func (e *NoopEngine) Uninstall(ctx context.Context, tenantID uint, pluginID string) error {
	log.Printf("[NoopEngine] 卸载插件: tenant=%d, plugin=%s", tenantID, pluginID)
	return nil
}

func (e *NoopEngine) Enable(ctx context.Context, tenantID uint, pluginID string) error {
	log.Printf("[NoopEngine] 启用插件: tenant=%d, plugin=%s", tenantID, pluginID)
	return nil
}

func (e *NoopEngine) Disable(ctx context.Context, tenantID uint, pluginID string) error {
	log.Printf("[NoopEngine] 禁用插件: tenant=%d, plugin=%s", tenantID, pluginID)
	return nil
}

func (e *NoopEngine) Upgrade(ctx context.Context, tenantID uint, pluginID string, oldVersion string, newVersion string, licenseKey string) error {
	log.Printf("[NoopEngine] 升级插件: tenant=%d, plugin=%s, %s→%s", tenantID, pluginID, oldVersion, newVersion)
	return nil
}

func (e *NoopEngine) GetStatus(ctx context.Context, tenantID uint, pluginID string) (*PluginStatus, error) {
	return &PluginStatus{
		PluginID: pluginID,
		Version:  "1.0.0",
		Status:   "active",
...
func (e *NoopEngine) ListPlugins(ctx context.Context, tenantID uint) ([]*InstalledPluginInfo, error) {
	return []*InstalledPluginInfo{}, nil
}

func (e *NoopEngine) Call(ctx context.Context, tenantID uint, pluginID string, functionName string, payload []byte) ([]byte, error) {
...
func (e *NoopEngine) HealthCheck(ctx context.Context, tenantID uint, pluginID string) error {
